...
const poParseHtml = async htmlData => {
    ...
    for (;;) {
        ...
        const poParsed = (await getPoPhrase(parseWithoutDollars, "gui"))
            ...
            .replace(/>/g, "&gt;")
            .replace(/"/g, "&quot;")
            .replace("{license}", '<a href="javascript:openMPL()" class="urlDisplay">MPL-2.0</a>')
            .replace("{acceleratorDocs}", `<a href="javascript:openAcceleratorDocs()" class="urlDisplay">${i18nThisDocumentation}</a>`)
