...
<script lang="ts">
    ...
    export default Vue.extend({
        ...
        data() {
            return {
                active: false,
                action: window.config.o.clipboard_action,
                fileConfigCheckboxI: window.config.o.save_capture || false,
                fileNamingPatternI: window.config.o.file_naming_pattern || "screenshot_%date%_%time%",
                fileSaveFolderI: window.config.o.save_path,
        ...
        methods: {
            ...
            saveSaveCapture() {
                window.config.o.save_capture = (document.getElementById("fileConfigCheckbox")! as HTMLInputElement).checked
                saveConfig()
            },
            saveNamingPattern() {
                const val = (document.getElementById("fileNamingPattern")! as HTMLInputElement).value
                this.$data.fileNamingPatternI = val
                window.config.o.file_naming_pattern = val
                saveConfig()
            },
            saveFilePath() {
